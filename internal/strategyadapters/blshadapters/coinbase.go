...
func NewCoinbaseAdapter(s blsh) *blshCoinbaseAdapter {
	return &blshCoinbaseAdapter{s}
}

func (a *blshCoinbaseAdapter) Reroute(data []byte) error {
		...
		for _, e := range msg.Events {
			if e.Type == "update" {
				for _, t := range e.Tickers {
					...
					a.Ticks() <- strategyprimitives.NewTick(t.ProductID, price, time)
				}
			}
		}
	default:
